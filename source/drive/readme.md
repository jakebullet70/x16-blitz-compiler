# `source/drive/` -- the emulator drive
...
**`GPC.ERR` is not here.** It builds in `GPC-BASIC-TOOLS-SRC/GPC.ERR/`, which is its own emulator drive and
...
|---|---|
| `*.INC.BL`, `<PROGRAM>.BASL` | copied in from `GPC-BASIC-TOOLS-SRC/<name>/GPC-BASIC/` and `GPC-BASIC/` before a build. The tracked copy is the master; one here that outlives a build is stale |
...
| `GPC-BASIC-TOOLS-SRC/` | `make samples`, mirroring the tracked `GPC-BASIC-TOOLS-SRC/` tree |
| `HELP-TXT/` | xcopied from `GPC-BASIC-TOOLS-SRC/GPC-HELP/HELP-TXT/` so `GPB.HELP.PRG` can read it off the drive |
