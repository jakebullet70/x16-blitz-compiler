...
# ************************************************************************************************
#
#		source/drive/MD5 is the Commodore BASIC MD5 from Rosetta Code (line 1100's GOTO retargeted --
...
SOURCE  = "MD5"                     # the BASIC program, tracked in source/drive/
...
def hash_with(program, logname):
    ...
    if not found:
        die("%s printed no digest (see source/drive/%s)" % (program, logname))
    return found[0].decode()


def main():
    for f, what in ((EMU, "emulator"), (ROM, "ROM"),
                    (os.path.join(TESTING, SOURCE), "source/drive/" + SOURCE),
                    (os.path.join(TESTING, "GPC.BIN"), "source/drive/GPC.BIN")):
