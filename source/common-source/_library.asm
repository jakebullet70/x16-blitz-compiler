;
;	This file is automatically generated
;
;
;	This file is automatically generated.
;
C64_END                  = $80 ; $80 end
C64_FOR                  = $81 ; $81 for
C64_NEXT                 = $82 ; $82 next
C64_DATA                 = $83 ; $83 data
C64_INPUTHASH            = $84 ; $84 input#
C64_INPUT                = $85 ; $85 input
C64_DIM                  = $86 ; $86 dim
C64_READ                 = $87 ; $87 read
C64_LET                  = $88 ; $88 let
C64_GOTO                 = $89 ; $89 goto
C64_RUN                  = $8a ; $8a run
C64_IF                   = $8b ; $8b if
C64_RESTORE              = $8c ; $8c restore
C64_GOSUB                = $8d ; $8d gosub
C64_RETURN               = $8e ; $8e return
C64_REM                  = $8f ; $8f rem
C64_STOP                 = $90 ; $90 stop
C64_ON                   = $91 ; $91 on
C64_WAIT                 = $92 ; $92 wait
C64_LOAD                 = $93 ; $93 load
C64_SAVE                 = $94 ; $94 save
C64_VERIFY               = $95 ; $95 verify
C64_DEF                  = $96 ; $96 def
C64_POKE                 = $97 ; $97 poke
C64_PRINTHASH            = $98 ; $98 print#
C64_PRINT                = $99 ; $99 print
C64_CONT                 = $9a ; $9a cont
C64_LIST                 = $9b ; $9b list
C64_CLR                  = $9c ; $9c clr
C64_CMD                  = $9d ; $9d cmd
C64_SYS                  = $9e ; $9e sys
C64_OPEN                 = $9f ; $9f open
C64_CLOSE                = $a0 ; $a0 close
C64_GET                  = $a1 ; $a1 get
C64_NEW                  = $a2 ; $a2 new
C64_TABLB                = $a3 ; $a3 tab(
C64_TO                   = $a4 ; $a4 to
C64_FN                   = $a5 ; $a5 fn
C64_SPCLB                = $a6 ; $a6 spc(
C64_THEN                 = $a7 ; $a7 then
C64_NOT                  = $a8 ; $a8 not
C64_STEP                 = $a9 ; $a9 step
C64_PLUS                 = $aa ; $aa +
C64_MINUS                = $ab ; $ab -
C64_TIMES                = $ac ; $ac *
C64_DIVIDE               = $ad ; $ad /
C64_POWER                = $ae ; $ae ^
C64_AND                  = $af ; $af and
C64_OR                   = $b0 ; $b0 or
C64_GREATER              = $b1 ; $b1 >
C64_EQUAL                = $b2 ; $b2 =
C64_LESS                 = $b3 ; $b3 <
C64_SGN                  = $b4 ; $b4 sgn
C64_INT                  = $b5 ; $b5 int
C64_ABS                  = $b6 ; $b6 abs
C64_USR                  = $b7 ; $b7 usr
C64_FRE                  = $b8 ; $b8 fre
C64_POS                  = $b9 ; $b9 pos
C64_SQR                  = $ba ; $ba sqr
C64_RND                  = $bb ; $bb rnd
C64_LOG                  = $bc ; $bc log
C64_EXP                  = $bd ; $bd exp
C64_COS                  = $be ; $be cos
C64_SIN                  = $bf ; $bf sin
C64_TAN                  = $c0 ; $c0 tan
C64_ATN                  = $c1 ; $c1 atn
C64_PEEK                 = $c2 ; $c2 peek
C64_LEN                  = $c3 ; $c3 len
C64_STRDOLLAR            = $c4 ; $c4 str$
C64_VAL                  = $c5 ; $c5 val
C64_ASC                  = $c6 ; $c6 asc
C64_CHRDOLLAR            = $c7 ; $c7 chr$
C64_LEFTDOLLAR           = $c8 ; $c8 left$
C64_RIGHTDOLLAR          = $c9 ; $c9 right$
C64_MIDDOLLAR            = $ca ; $ca mid$
C64_GO                   = $cb ; $cb go
C64_PI                   = $ff ; $ff pi
C64_GPCMD_BSTRSET        = $ce4d ; $ce4d gp.bstrset
C64_RETURNS              = $ce4e ; $ce4e returns
C64_GPCMD_FN             = $ce4f ; $ce4f gp.fn
C64_GPCMD_SUB            = $ce50 ; $ce50 gp.sub
C64_GPCMD_DEFPROC        = $ce51 ; $ce51 gp.defproc
C64_GPCMD_ENDBANKEDSTR   = $ce52 ; $ce52 gp.endbankedstr
C64_GPCMD_BANKEDSTR      = $ce53 ; $ce53 gp.bankedstr
C64_GPCMD_BSTRCOUNT      = $ce54 ; $ce54 gp.bstrcount
C64_GPCMD_BSTR           = $ce55 ; $ce55 gp.bstr
C64_GPCMD_ENDBANKED      = $ce56 ; $ce56 gp.endbanked
C64_GPCMD_BANKED         = $ce57 ; $ce57 gp.banked
C64_GPCMD_CHAR           = $ce58 ; $ce58 gp.char
C64_GPCMD_ENDASM         = $ce59 ; $ce59 gp.endasm
C64_GPCMD_ASM            = $ce5a ; $ce5a gp.asm
C64_GPCMD_ENDIF          = $ce5b ; $ce5b gp.endif
C64_GPCMD_ELSE           = $ce5c ; $ce5c gp.else
C64_GPCMD_ELSEIF         = $ce5d ; $ce5d gp.elseif
C64_GPCMD_IF             = $ce5e ; $ce5e gp.if
C64_GPCMD_ISEMPTY        = $ce5f ; $ce5f gp.isempty
C64_GPCMD_LOBYTE         = $ce60 ; $ce60 gp.lobyte
C64_GPCMD_HIBYTE         = $ce61 ; $ce61 gp.hibyte
C64_GPCMD_CONTAINS       = $ce62 ; $ce62 gp.contains
C64_GPCMD_ENDSEL         = $ce63 ; $ce63 gp.endsel
C64_GPCMD_OTHER          = $ce64 ; $ce64 gp.other
C64_GPCMD_CASE           = $ce65 ; $ce65 gp.case
C64_GPCMD_SELECT         = $ce66 ; $ce66 gp.select
C64_GPCMD_PRINTAT        = $ce69 ; $ce69 gp.printat
C64_GPCMD_FILL           = $ce6a ; $ce6a gp.fill
C64_GPCMD_BOX            = $ce6b ; $ce6b gp.box
C64_GPCMD_ARRPTR         = $ce6e ; $ce6e gp.arrptr
C64_GPCMD_COMP           = $ce70 ; $ce70 gp.comp
C64_GPCMD_STRPTR         = $ce76 ; $ce76 gp.strptr
C64_GPCMD_INSTR          = $ce77 ; $ce77 gp.instr
C64_GPCMD_C              = $ce78 ; $ce78 gp.c
C64_GPCMD_Y              = $ce79 ; $ce79 gp.y
C64_GPCMD_X              = $ce7a ; $ce7a gp.x
C64_GPCMD_A              = $ce7b ; $ce7b gp.a
C64_GPCMD_CALL           = $ce7c ; $ce7c gp.call
C64_GPCMD_EXITDO         = $ce7d ; $ce7d gp.exitdo
C64_GPCMD_LOOP           = $ce7e ; $ce7e gp.loop
C64_GPCMD_DO             = $ce7f ; $ce7f gp.do
C64_MON                  = $ce80 ; $ce80 mon
C64_DOS                  = $ce81 ; $ce81 dos
C64_OLD                  = $ce82 ; $ce82 old
C64_GEOS                 = $ce83 ; $ce83 geos
C64_VPOKE                = $ce84 ; $ce84 vpoke
C64_VLOAD                = $ce85 ; $ce85 vload
C64_SCREEN               = $ce86 ; $ce86 screen
C64_PSET                 = $ce87 ; $ce87 pset
C64_LINE                 = $ce88 ; $ce88 line
C64_FRAME                = $ce89 ; $ce89 frame
C64_RECT                 = $ce8a ; $ce8a rect
C64_CHAR                 = $ce8b ; $ce8b char
C64_MOUSE                = $ce8c ; $ce8c mouse
C64_COLOR                = $ce8d ; $ce8d color
C64_TEST                 = $ce8e ; $ce8e test
C64_RESET                = $ce8f ; $ce8f reset
C64_CLS                  = $ce90 ; $ce90 cls
C64_CODEX                = $ce91 ; $ce91 codex
C64_LOCATE               = $ce92 ; $ce92 locate
C64_BOOT                 = $ce93 ; $ce93 boot
C64_KEYMAP               = $ce94 ; $ce94 keymap
C64_BLOAD                = $ce95 ; $ce95 bload
C64_BVLOAD               = $ce96 ; $ce96 bvload
C64_BVERIFY              = $ce97 ; $ce97 bverify
C64_BANK                 = $ce98 ; $ce98 bank
C64_FMINIT               = $ce99 ; $ce99 fminit
C64_FMNOTE               = $ce9a ; $ce9a fmnote
C64_FMDRUM               = $ce9b ; $ce9b fmdrum
C64_FMINST               = $ce9c ; $ce9c fminst
C64_FMVIB                = $ce9d ; $ce9d fmvib
C64_FMFREQ               = $ce9e ; $ce9e fmfreq
C64_FMVOL                = $ce9f ; $ce9f fmvol
C64_FMPAN                = $cea0 ; $cea0 fmpan
C64_FMPLAY               = $cea1 ; $cea1 fmplay
C64_FMCHORD              = $cea2 ; $cea2 fmchord
C64_FMPOKE               = $cea3 ; $cea3 fmpoke
C64_PSGINIT              = $cea4 ; $cea4 psginit
C64_PSGNOTE              = $cea5 ; $cea5 psgnote
C64_PSGVOL               = $cea6 ; $cea6 psgvol
C64_PSGWAV               = $cea7 ; $cea7 psgwav
C64_PSGFREQ              = $cea8 ; $cea8 psgfreq
C64_PSGPAN               = $cea9 ; $cea9 psgpan
C64_PSGPLAY              = $ceaa ; $ceaa psgplay
C64_PSGCHORD             = $ceab ; $ceab psgchord
C64_REBOOT               = $ceac ; $ceac reboot
C64_POWEROFF             = $cead ; $cead poweroff
C64_I2CPOKE              = $ceae ; $ceae i2cpoke
C64_SLEEP                = $ceaf ; $ceaf sleep
C64_BSAVE                = $ceb0 ; $ceb0 bsave
C64_MENU                 = $ceb1 ; $ceb1 menu
C64_REN                  = $ceb2 ; $ceb2 ren
C64_LINPUTHASH           = $ceb3 ; $ceb3 linput#
C64_LINPUT               = $ceb4 ; $ceb4 linput
C64_BINPUTHASH           = $ceb5 ; $ceb5 binput#
C64_HELP                 = $ceb6 ; $ceb6 help
C64_BANNER               = $ceb7 ; $ceb7 banner
C64_EXEC                 = $ceb8 ; $ceb8 exec
C64_TILE                 = $ceb9 ; $ceb9 tile
C64_EDIT                 = $ceba ; $ceba edit
C64_SPRITE               = $cebb ; $cebb sprite
C64_SPRMEM               = $cebc ; $cebc sprmem
C64_MOVSPR               = $cebd ; $cebd movspr
C64_BASLOAD              = $cebe ; $cebe basload
C64_OVAL                 = $cebf ; $cebf oval
C64_RING                 = $cec0 ; $cec0 ring
C64_HBLOAD               = $cec1 ; $cec1 hbload
C64_VPEEK                = $ced0 ; $ced0 vpeek
C64_MX                   = $ced1 ; $ced1 mx
C64_MY                   = $ced2 ; $ced2 my
C64_MB                   = $ced3 ; $ced3 mb
C64_JOY                  = $ced4 ; $ced4 joy
C64_HEXDOLLAR            = $ced5 ; $ced5 hex$
C64_BINDOLLAR            = $ced6 ; $ced6 bin$
C64_I2CPEEK              = $ced7 ; $ced7 i2cpeek
C64_POINTER              = $ced8 ; $ced8 pointer
C64_STRPTR               = $ced9 ; $ced9 strptr
C64_RPTDOLLAR            = $ceda ; $ceda rpt$
C64_MWHEEL               = $cedb ; $cedb mwheel
C64_TDATA                = $cedc ; $cedc tdata
C64_TATTR                = $cedd ; $cedd tattr
C64_MOD                  = $cede ; $cede mod
; ************************************************************************************************
; ************************************************************************************************
;
;		Name:		common.inc
;		Purpose:	Common includes/defines/setups
;		Created:	11th April 2023
;		Reviewed: 	No
;		Author:		Paul Robson (paul@robsons.org.uk)
;
; ************************************************************************************************
; ************************************************************************************************

; ************************************************************************************************
;
;										Configuration options
;
;			  Addresses should be on page boundaries (except for ZeroPageMandatory)
;
; ************************************************************************************************
;
;		Where to assemble the runnable code. Happy now , Dave ?
;
;		Weak so the standalone resident-runtime build (GPC.RT.nnn.BIN) can override it to RTBASE
;		with -D CodeStart on the command line; every other build keeps $801.
;
		.weak
CodeStart = $801
		.endweak
;
;		Variables that have to be in zero page because it's used in (xx),y
;
ZeroPageMandatory = $22
;
;		Variables that can go anywhere
;
MemoryStorage = $400
;
;		Resident-runtime (GPC.RT.nnn.BIN) ABI -- see application/source/compiler/bootstrap.asm and
;		runtime/source/main/00rt.header. RTBASE is the fixed home the standalone runtime is linked
;		at and loaded to; a shared-mode program's bootstrap checks the magic there, loads it if
;		absent, and enters at RT_ENTRY. RTBASE is PINNED (baked into every shared program's
;		bootstrap), so it is an ABI constant, not auto-computed; zzrt.footer guards it against the
;		runtime outgrowing $9F00.
;
;
;		RTBASE moved $7300 -> $7000 on 31st July 2026 (RT_ABI 1 -> 2). The runtime had outgrown the
;		$9F00 I/O page by 447 bytes and zzrt.footer's guard had been failing the gpc-rt link for
;		some time -- shared mode could not be built at all. Three pages down frees 768 bytes and
;		leaves ~320 spare, at the cost of the same 768 bytes of per-program workspace (~3% of the
;		~26K a shared program gets). If it happens again, lower this and bump RT_ABI together.
;
;
;		AND AGAIN on 16th August 2026, exactly as warned above. The runtime had crept to $9EFF --
;		ONE byte under the ceiling -- so the ~200 bytes added by ReuseForFrame, GP.EXITDO and
;		GP.CALL overflowed it.
;
;		EIGHT pages down this time, not the three that just proved too timid. Every move of RTBASE
;		costs an RT_ABI bump (a resident runtime is keyed on the ordinal, and one built at another
;		base is not interchangeable), so the cheap moment to take the room is while a bump is
;		already happening -- RT_ABI was going 5 -> 6 for GP.CALL's opcodes regardless. Four pages
;		would have left 940 bytes, which does NOT cover the ~1,545 GP.BASIC still has to add;
;		eight leaves ~1,960 and ends the matter. Cost is 2K of the ~26K a shared program gets.
;
; ************************************************************************************************
;
;		AND A THIRD TIME on 17th August 2026: $6800 -> $6400, four pages, BEFORE hitting the wall
;		rather than after. The runtime had 1,061 bytes left under the ceiling and the rest of
;		GP.BASIC (stash/restore, drawing, the menu loop) projects to ~1,400 -- measured, not
;		guessed: the estimates in GP-BASIC.TIERS.md have run 1.62x over on average, with GP.SORT
;		at 2.55x and GP.ARRPTR at 3.27x.
;
;		Four pages, not the two that would just cover the projection, for the reason written twice
;		above: three pages "proved too timid" and the note then said take the room while a bump is
;		already free. Leaves ~690 bytes of margin after everything planned. Costs 1K of the ~26K a
;		shared program gets, about 4%.
;
; ************************************************************************************************
; ************************************************************************************************
;
;		SPLIT IN TWO on 18th August 2026 (RT_ABI 18 -> 19), so a shared program that uses no GPB
;		keyword does not have to keep the GPB handlers out of its workspace.
;
;		The GPB block is now linked at the BOTTOM of the shared image and the interpreter core
;		above it, at a base that does not move:
;
;			RTGPBASE  $6F00   [ GPB handlers ..................... ][ GP magic ]
;			RTBASE    $7700   [ magic ][ jmp StartRuntime ][ core ... ] up to ~$9D80
;
;		ONE assembly still, and one vector table -- the two FILES rtname.py installs are slices
;		of the same image, so the core's bytes are identical in both and there is no second
;		build to keep in step:
;
;			GPC.RT.nnn.BIN   loads at RTGPBASE -- GPB handlers AND core
;			GPC.RC.nnn.BIN   loads at RTBASE   -- core only
;
;		A program's workspace then ends at RTBASE if it uses no GPB keyword and at RTGPBASE if it
;		does, which is 2,560 bytes back for the first kind and no change at all for the second.
;
;		WHY THE CORE IS ON TOP and not the other way round: the core has to sit at ONE address
;		whichever file was loaded, or a program could not enter a runtime the previous program
;		brought up. Only the OPTIONAL half can be the half that moves the boundary, so the
;		optional half has to be the low one.
;
;		RTGPMAGIC is how a GPB program tells the two apart. The core magic at RTBASE says "a
;		runtime is up"; it cannot also say "and the handlers are with it", because a core-only
;		load leaves the core magic looking exactly the same. So the GPB half carries its own
;		magic in the four bytes immediately below RTBASE, and a GPB program checks both. It sits
;		THERE rather than at RTGPBASE so a core-only program's workspace -- which runs up to
;		RTBASE -- overwrites it as a matter of course, and the next GPB program correctly finds
;		the handlers gone and reloads them.
;
; ************************************************************************************************
RTGPBASE = $6F00 							; GPB handler block + load address of the FULL file
RTBASE = $7700 								; interpreter core home -- the base that never moves
RT_ENTRY = RTBASE+4 						; 4-byte magic at RTBASE, then jmp StartRuntime
RTGPMAGIC = RTBASE-4 						; 4-byte "handlers are loaded too" magic

;
;		GP.BSTR's text banks: SIXTEEN, one to a slot, handed from the program to the runtime.
;
;		A FIXED ADDRESS AND NOT A STORAGE LABEL, and the reason is worth writing down. The runtime
;		is linked TWICE with gp.library at opposite ends -- last in the application's embedded
;		image, first in the standalone GPB.RT file -- so every label in the GPB half lands at a
;		different address in the two. A handover through a label works in whichever link the
;		bootstrap was told about and reads a stale byte in the other: here it was $05E5 against
;		$0400, and the program ran, printed garbage, and blamed the handler.
;
;		IT IS IN THE STORAGE HOLE, $0400-$0801, AT THE TOP OF IT. That hole is the one piece of
;		low RAM both builds have and neither occupies: a program's image starts at $0801 and its
;		workspace at $0900 or higher, so nothing loads over it and the runtime never clears it.
;		StorageEnd measures $05F0 in both runtime links, which leaves 528 bytes between the
;		sections and this table.
;
;		IT USED TO BE $09F0, the top of the bootstrap extension page, where it arrived loaded and
;		no code moved it. That address is p-code in a shared program with no region, which was
;		harmless -- such a program has no GP.BANKEDSTR -- but it is the RUNTIME IMAGE ITSELF in
;		an embedded one, which is not. Embedded was refused for that reason alone. The table moved
;		down here instead, and the two loaders each copy sixteen bytes into it: bootstrap2.asm at
;		BXRun out of the extension page it already carries, 00rtimage.header at StartCode out of
;		the image. Eleven bytes of copy in each, and GP.BANKEDSTR compiles in both modes.
;
;		THE COPY IS ON THE RE-RUN PATH IN BOTH, deliberately. A second RUN reloads no region and
;		re-reads no overlay, and this table would survive untouched anyway -- but it costs eleven
;		bytes to put back rather than to reason about, and a program that scribbled on it gets it
;		back.
;
;		THE ADDRESS IS PINNED BY THIS CONSTANT AND CHECKED AT BOTH ENDS -- common.inc .cerrors if
;		a link's storage section has grown into it, and genrtimage.py .cerrors if the image's own
;		copy of it straddles a page of the stream the object writer patches.
;
;		SIXTEEN, because the slot is the top FOUR BITS of the constant every GP.BSTR call site
;		already pushes -- see compiler/commands/gpbstr.asm. Four bits leaves twelve for the index,
;		and 4,096 is above the 2,047 strings an 8K bank can physically hold (two directory bytes
;		and a capacity and a length byte each), so the region's own 8K check always fires first
;		and the index field caps nothing.
;
BSTR_MAX_BANKS = 16 						; text banks a program may have, = the slot field's range
GPBSTRBANKS = $0800 - BSTR_MAX_BANKS 		; the table, at the top of the storage hole ($07F0)
HANDLER_BANK = 1							; the runtime's rarely used handlers; no region may use it
PCODE_PAGE = $09 							; shared-mode p-code base page ($0900, page-aligned)
MIN_WS_PAGES = 16 							; smallest workspace a shared program keeps (4K)
;
;		THE BYTE AFTER THE LAST REGION in an overlay, which is what tells a reader it has them
;		all. Bank 1 is the runtime's and the compiler refuses GP.BANKED 1, so no region's bank
;		byte is ever this. object.asm writes it at ObjStreamClose.
;
;		HERE, because both links read it now: bootstrap2.asm walks a .OVL for a shared program
;		and 00rtimage.header walks the same bytes in RAM for an embedded one.
;
BXOVLEND = 1
;
;		THE OVERLAY'S LOGICAL FILE, and the longest name a reader may be handed. Both readers
;		want both numbers: bootstrap2.asm opens a .OVL for a shared program and 00rtimage.header
;		opens one for an embedded program that has regions, so neither may drift from the other.
;
;		BXFILE IS A DATA CHANNEL, not a LOAD: the file has no load address of its own and does not
;		go to one place. Not 1 -- file 1 has been seen to hang a later OPEN.
;
;		BXNAMEMAX IS 32 BECAUSE THE BOOTSTRAP EXTENSION PAGE SAYS SO, and the runtime image copied
;		the bound rather than pick its own. A CMDR-DOS file name is 16 characters, so 32 holds one
;		with a drive prefix and a ".OVL" on it. The compiler refuses a longer one rather than
;		truncating it -- see _WOCSExtNameLong.
;
BXFILE = 2
BXNAMEMAX = 32
;
;		Pages left free between the end of the object code and the start of the workspace, for
;		the runtime (FOR/GOSUB) frame stack. clr.asm puts runtimeStackPtr at storeStartHigh-1:$FF
;		and grows it DOWNWARD, so without a gap here a deep call chain walks back into the code.
;
;		BOTH ends need this number and they must agree, which is why it lives here rather than
;		with either of them. The COMPILER spends it -- object.asm adds it when it works out where
;		a program's workspace starts, in both embedded and shared modes. The RUNTIME polices it --
;		StartRuntime turns storeStartHigh minus this into the floor StackOpenFrame refuses to go
;		below. Change it in one place and a program compiled by the old engine, running under a
;		new runtime, gets a floor that is not where its gap is.
;
;		EIGHT, not sixteen, since 08/09/26. Four kilobytes was set when a program was small and
;		its workspace was most of the machine; it is now most of the workspace. GPBMODS gets
;		$5400..$6600 = 4,608 bytes to run in, of which 2,282 are its scalars, and the frame stack
;		below it was very nearly as big as the whole thing -- so FILES/DIR OPEN ran out of string
;		heap building twenty-four listbox rows and reported OUT OF MEMORY from inside STR.PADR.
;
;		Two kilobytes is still ~125 GOSUB frames (4 bytes), ~107 FOR frames (19, the largest), and
;		nothing in this tree recurses. Overflow is not silent either way: StackOpenFrame tests
;		stackFloorHigh and reports OUT OF MEMORY, which is what stock BASIC says for too many
;		nested GOSUBs. So the trade is depth nobody uses for 2,048 bytes every program can spend.
;
FrameStackPages = 8 						; 2K, ~125 frames
;
;		RT_ABI is the runtime's ABI ordinal, and it is the ONE place it is written down. It drives
;		the 4-byte magic at RTBASE ("GPC" + the digit), which the bootstrap compares to answer a
;		single question: is a runtime already resident, and is it one I can safely enter? So bump
;		it whenever the runtime layout or entry contract changes -- and only then. A resident
;		runtime from a DIFFERENT engine build but the same ABI is deliberately still reused.
;
;		The runtime's FILE NAME does not come from here. It carries the engine's BUILD number
;		(source/application/buildnum.txt) -- see scripts/rtname.py, which names the built file,
;		and bootstrap.asm, which formats the same number into the name a compiled program looks
;		for. That is a different question: which runtime file is mine? It pins a program to the
;		exact runtime it was built against, and since the build number bumps on every engine
;		build, shared programs must be recompiled whenever the engine is rebuilt.
;
;		2 -> 3 on 16th August 2026: PCD_ARRAY1 joined the table, and p-code IDs are handed out in
;		the order the ;; [name] markers are scanned, so every opcode after "array1" shifted up by
;		one. A resident runtime of the same ABI is deliberately reused across engine builds --
;		and with renumbered opcodes that would dispatch most of the table to the wrong handler.
;		This is the guard that stops it.
;
;		3 -> 4 on 16th August 2026: GP.DO / GP.LOOP joined the table. Same mechanism as the 2 -> 3
;		bump above -- "do.asm" sorts ahead of "for.asm" in the commands directory, so the two new
;		markers landed at $96/$97 and shifted every opcode from "for" upward by two.
;
;		4 -> 5 on 16th August 2026: GP.EXITDO joined the table as the system token ".exitdo". This
;		one is NOT a renumbering -- it was appended after ".deferror" precisely so that every
;		existing opcode keeps its ID. The bump is still required in the other direction: an object
;		using $DF would dispatch off the end of an ABI 4 runtime's vector table.
;
;		5 -> 6 on 16th August 2026: GP.CALL and the GP.A/GP.X/GP.Y/GP.C value words. This one IS a
;		renumbering -- "gpcall.asm" sorts mid-list in the commands directory, so the four unshifted
;		markers shifted every opcode above them, and the shifted gp.call did the same to the
;		shifted range.
;
;		6 -> 7 on 16th August 2026: GP.FIND and GP.STRPTR. "gpstring.asm" sorts after "gpcall.asm",
;		so the two new unshifted markers shifted every opcode above them again.
;
;		7 -> 8 on 16th August 2026: GP.TRIM / GP.UPPER / GP.LOWER / GP.PAD joined gpstring.asm.
;
;		8 -> 9 on 16th August 2026: GP.PAD REMOVED again -- see gpstring.asm for why. Dropping a
;		marker renumbers exactly as adding one does, so every shifted opcode above gp.pad moved
;		back down. ABI 8 was never committed, so this bump is belt and braces rather than
;		necessity -- but GPC.RT.*.BIN files built during the day are still lying about on this
;		machine carrying the "GPC8" magic and the OLD opcode map, and the bootstrap would accept
;		one for a new ABI 8 object without a word. That is the silent-mismatch class this ordinal
;		exists to prevent, so it gets spent rather than argued with. Rebuild the resident runtime.
;
;		9 -> 10 on 17th August 2026: the magic at RTBASE widened from "GPC"+one digit to "GP"+TWO
;		digits. It is still exactly 4 bytes (a 5th would have overwritten the RT_ENTRY jmp at
;		RTBASE+4, which is what the .cerror in 00rt.header and bootstrap.asm was guarding), and it
;		now has room to ABI 99. GP.INSTRREV was added and then removed again under this same
;		ordinal; neither state was committed, so 10 means the two-digit magic and nothing else.
;
;		10 -> 11 on 17th August 2026: GP.COMP joined gpstring.asm -- a shifted marker, so every
;		shifted opcode above it moved up one.
;
;		11 -> 12 on 17th August 2026: GP.SORT, in its own gpsort.asm. That file sorts between
;		"gpstring.asm" and "gpcall.asm" in the commands directory, so it renumbered from there up.
;
;		12 -> 13 on 17th August 2026: GP.ARRPTR joined gpsort.asm, a shifted marker.
;
;		13 -> 14 on 17th August 2026: RTBASE moved $6800 -> $6400. Every move of the base costs a
;		bump -- a resident runtime linked at another address is not interchangeable, and the base is
;		baked into every shared program's bootstrap.
;
;		14 -> 15 on 17th August 2026: GP.STASH / GP.RESTR joined the x16 commands directory.
;
;		15 -> 16 on 17th August 2026: GP.BOX / GP.FILL / GP.PRINTAT, in gpdraw.asm. That leafname
;		sorts BEFORE gpsort.asm and gpstash.asm, so the three new shifted markers were inserted
;		below theirs rather than appended, and everything from gp.sort upwards moved by three.
;
;		16 -> 17 on 17th August 2026: GP.MENU / GP.SEL, in gpmenu.asm. "gpmenu" sorts between
;		"gpdraw" and "gpsort", so the two new shifted markers were inserted rather than appended
;		and everything from gp.sort upwards moved by two.
;
;		17 -> 18 on 17th August 2026: GP.SELECT / GP.CASE / GP.ELSE / GP.ENDSEL, in commands/
;		select.asm. This is the biggest renumbering since GP.CALL: four UNSHIFTED markers, and
;		"select.asm" sorts mid-list in the commands directory, so every unshifted opcode above
;		them moved by four -- which moved PCD_STARTSYSTEM, so the whole system-token block and
;		with it the shifted base ($D780 -> $DB80) moved too. Two system tokens (.casenext,
;		.caseend) were appended after .exitdo on top of that.
;
;		20 -> 21 on 30th August 2026: GP.IF / GP.ELSEIF / GP.ELSE / GP.ENDIF, and GP.SELECT's
;		GP.ELSE renamed GP.OTHER to free the spelling. Two UNSHIFTED markers (gp.if, gp.endif)
;		go in commands/goto.asm -- deliberately the CORE, not gp-runtime, so ScanGPUsage does
;		not drag the 2K GP block into a program whose only GP keyword is an IF. "goto.asm"
;		sorts early, so every unshifted opcode above it moved by two, which moved
;		PCD_STARTSYSTEM and with it the shifted base ($DB80 -> $DD80). Two system tokens
;		(.ifnext, .ifelse) were appended after .caseend on top of that. The gp.else -> gp.other
;		rename moved nothing: same position, same id.
;
;		23 -> 24 on 15th September 2026: rarely used handlers moved to bank 1 (HANDLER-BANK). The move
;		renumbered ten opcodes (CHAR, PSET to RING, MOVSPR, SPRITE and SPRMEM), 40 vector entries now
;		go to BankEnter or BankEnterShift, and the bank code carries this ordinal in a magic at $A000.
;		The core shrank enough to move RTGPBASE and RTBASE UP nine pages, $6600 -> $6F00 and $6E00 ->
;		$7700: 2,304 bytes more workspace for every shared program, and 379 left below $9F00.
;
;		24 -> 25 on 18th September 2026: GP.BSTRSET. Its shifted marker follows gp.bstr in
;		gp-runtime's gpbstr.asm, so every shifted opcode from gp.call up moved by one. A text bank
;		record gained a capacity byte in the same change.
;
RT_ABI = 25 								; runtime ABI ordinal -> "GP25" magic (NOT the file name)

; ************************************************************************************************
;
;									Set up code and data sections
;
; ************************************************************************************************

		* = ZeroPageMandatory 				; *must* be in zero page
		.dsection zeropage

		* = MemoryStorage 					; doesn't matter if zero page or not
		.dsection storage
StorageEnd:

		* = CodeStart
		.dsection code

;
;		storage is a 1K hole -- $0400 to $0801, and no further, because the code starts there.
;		Its top sixteen bytes are GPBSTRBANKS and are not the section's to take, so the guard is
;		against that and not against CodeStart. StorageEnd measures $05F0 in both runtime links
;		and $0678 in the compiler, so there is room; the message names the table because growing
;		into it is the failure that has no symptom.
;		Nothing warns you when it runs out: the section simply carries on over the top of the
;		BASIC stub, and the program corrupts itself the first time it writes to the buffer that
;		spilled. It HAD run out, silently, and only survived because the one buffer hanging over
;		the edge was never filled past its fifteenth byte. So say so, loudly.
;
		.cerror StorageEnd > GPBSTRBANKS, "storage has overflowed into the GP.BSTR bank table at $07F0 - put compiler-only buffers in the code section (see application/source/file-io/read.asm)"

; ************************************************************************************************
;
;										Zero Page Common Usage
;
; ************************************************************************************************

		.section zeropage

codePtr:	 								; address of current line.
		.fill 	2

objPtr: 									; address in code. 
		.fill 	2 							

zTemp0: 									; temporary words used in the interpreter.
		.fill 	2
zTemp1:
		.fill 	2
zTemp2:
		.fill 	2

		.endsection

; ************************************************************************************************
;
;							Insert an Emulator Breakpoint
;
; ************************************************************************************************

debug 		.macro
		.byte 	$DB 						; causes a break in the emulator
		.endm

; ************************************************************************************************
;
;								Set a 2 byte value in memory
;
; ************************************************************************************************

set16 		.macro
		lda 	#((\2) & $FF)
		sta 	0+\1
		lda 	#((\2) >> 8) & $FF
		sta 	1+\1
		.endm

; ************************************************************************************************
;
;										Exit emulation
;
; ************************************************************************************************

exitemu .macro
		stx 	zTemp0
		jmp 	$FFFF
		.endm

; ************************************************************************************************
;
;									Changes and Updates
;
; ************************************************************************************************
;
;		Date			Notes
;		==== 			=====
;
; ************************************************************************************************
;
;	This file is automatically generated.
;
error_range .macro
	jmp	ErrorV_range
	.endm
error_value .macro
	jmp	ErrorV_value
	.endm
error_syntax .macro
	jmp	ErrorV_syntax
	.endm
error_type .macro
	jmp	ErrorV_type
	.endm
error_unimplemented .macro
	jmp	ErrorV_unimplemented
	.endm
error_assert .macro
	jmp	ErrorV_assert
	.endm
error_line .macro
	jmp	ErrorV_line
	.endm
error_internal .macro
	jmp	ErrorV_internal
	.endm
error_divzero .macro
	jmp	ErrorV_divzero
	.endm
error_structure .macro
	jmp	ErrorV_structure
	.endm
error_stop .macro
	jmp	ErrorV_stop
	.endm
error_data .macro
	jmp	ErrorV_data
	.endm
error_undeclared .macro
	jmp	ErrorV_undeclared
	.endm
error_redefine .macro
	jmp	ErrorV_redefine
	.endm
error_index .macro
	jmp	ErrorV_index
	.endm
error_memory .macro
	jmp	ErrorV_memory
	.endm
error_channel .macro
	jmp	ErrorV_channel
	.endm
error_break .macro
	jmp	ErrorV_break
	.endm
error_toobig .macro
	jmp	ErrorV_toobig
	.endm
;
;	This file is automatically generated
;
PCD_STARTBINARY = $80
PCD_ENDBINARY = $8d
PCD_STARTCOMMAND = $8d
PCD_ENDCOMMAND = $dd
PCD_STARTSYSTEM = $dd
PCD_ENDSYSTEM = $f6

PCD_PLUS             = $80 ; +
PCD_MINUS            = $81 ; -
PCD_TIMES            = $82 ; *
PCD_DIVIDE           = $83 ; /
PCD_POWER            = $84 ; ^
PCD_AND              = $85 ; and
PCD_OR               = $86 ; or
PCD_GREATER          = $87 ; >
PCD_EQUAL            = $88 ; =
PCD_LESS             = $89 ; <
PCD_GREATEREQUAL     = $8a ; >=
PCD_LESSGREATER      = $8b ; <>
PCD_LESSEQUAL        = $8c ; <=
PCD_ABS              = $8d ; abs
PCD_ARRAY1           = $8e ; array1
PCD_ARRAY            = $8f ; array
PCD_ASC              = $90 ; asc
PCD_ASSERT           = $91 ; assert
PCD_BINDOLLAR        = $92 ; bin$
PCD_PRINTCMD_CHR     = $93 ; print.chr
PCD_CHRDOLLAR        = $94 ; chr$
PCD_SCMD_CMP         = $95 ; s.cmp
PCD_GPCMD_DO         = $96 ; gp.do
PCD_GPCMD_LOOP       = $97 ; gp.loop
PCD_FOR              = $98 ; for
PCD_FRE              = $99 ; fre
PCD_GET              = $9a ; get
PCD_RETURN           = $9b ; return
PCD_GPCMD_IF         = $9c ; gp.if
PCD_GPCMD_ENDIF      = $9d ; gp.endif
PCD_GPCMD_SELECT     = $9e ; gp.select
PCD_GPCMD_CASE       = $9f ; gp.case
PCD_GPCMD_OTHER      = $a0 ; gp.other
PCD_GPCMD_ENDSEL     = $a1 ; gp.endsel
PCD_GPCMD_A          = $a2 ; gp.a
PCD_GPCMD_X          = $a3 ; gp.x
PCD_GPCMD_Y          = $a4 ; gp.y
PCD_GPCMD_C          = $a5 ; gp.c
PCD_GPCMD_INSTR      = $a6 ; gp.instr
PCD_GPCMD_STRPTR     = $a7 ; gp.strptr
PCD_CHAR             = $a8 ; char
PCD_PSET             = $a9 ; pset
PCD_LINE             = $aa ; line
PCD_RECT             = $ab ; rect
PCD_FRAME            = $ac ; frame
PCD_OVAL             = $ad ; oval
PCD_RING             = $ae ; ring
PCD_HEXDOLLAR        = $af ; hex$
PCD_INPUT            = $b0 ; input
PCD_INPUTDOLLAR      = $b1 ; input$
PCD_INPUTCMD_START   = $b2 ; input.start
PCD_LEN              = $b3 ; len
PCD_FCMD_CMP         = $b4 ; f.cmp
PCD_INTCMD_DIV       = $b5 ; int.div
PCD_MOD              = $b6 ; mod
PCD_NEGATE           = $b7 ; negate
PCD_NEWCMD_LINE      = $b8 ; new.line
PCD_NEXT             = $b9 ; next
PCD_NOT              = $ba ; not
PCD_ON               = $bb ; on
PCD_MOREON           = $bc ; moreon
PCD_PEEK             = $bd ; peek
PCD_PI               = $be ; pi
PCD_POKE             = $bf ; poke
PCD_POS              = $c0 ; pos
PCD_GETCHANNEL       = $c1 ; getchannel
PCD_SETCHANNEL       = $c2 ; setchannel
PCD_PRINTCMD_N       = $c3 ; print.n
PCD_PRINTCMD_S       = $c4 ; print.s
PCD_READ             = $c5 ; read
PCD_READDOLLAR       = $c6 ; read$
PCD_RND              = $c7 ; rnd
PCD_CONCAT           = $c8 ; concat
PCD_SGN              = $c9 ; sgn
PCD_PRINTCMD_TAB     = $ca ; print.tab
PCD_PRINTCMD_POS     = $cb ; print.pos
PCD_PRINTCMD_SPC     = $cc ; print.spc
PCD_STRDOLLAR        = $cd ; str$
PCD_LEFTDOLLAR       = $ce ; left$
PCD_RIGHTDOLLAR      = $cf ; right$
PCD_MIDDOLLAR        = $d0 ; mid$
PCD_SWAP             = $d1 ; swap
PCD_TI               = $d2 ; ti
PCD_TIDOLLAR         = $d3 ; ti$
PCD_USR              = $d4 ; usr
PCD_VAL              = $d5 ; val
PCD_CLOSE            = $d6 ; close
PCD_EXIT             = $d7 ; exit
PCD_DEBUG            = $d8 ; debug
PCD_OPEN             = $d9 ; open
PCD_SCREEN           = $da ; screen
PCD_VPOKE            = $db ; vpoke
PCD_VPEEK            = $dc ; vpeek
PCD_CMD_SHIFT        = $dd ; .shift
PCD_CMD_BYTE         = $de ; .byte
PCD_CMD_WORD         = $df ; .word
PCD_CMD_FLOAT        = $e0 ; .float
PCD_CMD_STRING       = $e1 ; .string
PCD_CMD_DATA         = $e2 ; .data
PCD_CMD_GOTO         = $e3 ; .goto
PCD_CMD_GOSUB        = $e4 ; .gosub
PCD_CMD_GOTOCMD_Z    = $e5 ; .goto.z
PCD_CMD_GOTOCMD_NZ   = $e6 ; .goto.nz
PCD_CMD_VARSPACE     = $e7 ; .varspace
PCD_CMD_RESTORE      = $e8 ; .restore
PCD_CMD_FNGOSUB      = $e9 ; .fngosub
PCD_CMD_DEFERROR     = $ea ; .deferror
PCD_CMD_EXITDO       = $eb ; .exitdo
PCD_CMD_CASENEXT     = $ec ; .casenext
PCD_CMD_CASEEND      = $ed ; .caseend
PCD_CMD_IFNEXT       = $ee ; .ifnext
PCD_CMD_IFELSE       = $ef ; .ifelse
PCD_CMD_UNWIND       = $f0 ; .unwind
PCD_CMD_FNSAVE       = $f1 ; .fnsave
PCD_CMD_FNRESTORE    = $f2 ; .fnrestore
PCD_CMD_FNPUSH       = $f3 ; .fnpush
PCD_CMD_FNPOP        = $f4 ; .fnpop
PCD_CMD_BGOSUB       = $f5 ; .bgosub
PCD_CLR              = $dd80 ; clr
PCD_DIM              = $dd81 ; dim
PCD_END              = $dd82 ; end
PCD_GPCMD_ARRPTR     = $dd83 ; gp.arrptr
PCD_GPCMD_BSTR       = $dd84 ; gp.bstr
PCD_GPCMD_BSTRSET    = $dd85 ; gp.bstrset
PCD_GPCMD_CALL       = $dd86 ; gp.call
PCD_GPCMD_BOX        = $dd87 ; gp.box
PCD_GPCMD_FILL       = $dd88 ; gp.fill
PCD_GPCMD_PRINTAT    = $dd89 ; gp.printat
PCD_GPCMD_COMP       = $dd8a ; gp.comp
PCD_JOY              = $dd8b ; joy
PCD_INT              = $dd8c ; int
PCD_SQR              = $dd8d ; sqr
PCD_LOG              = $dd8e ; log
PCD_EXP              = $dd8f ; exp
PCD_COS              = $dd90 ; cos
PCD_SIN              = $dd91 ; sin
PCD_TAN              = $dd92 ; tan
PCD_ATN              = $dd93 ; atn
PCD_LINPUT           = $dd94 ; linput
PCD_BINPUT           = $dd95 ; binput
PCD_LOAD             = $dd96 ; load
PCD_BLOAD            = $dd97 ; bload
PCD_BVLOAD           = $dd98 ; bvload
PCD_VLOAD            = $dd99 ; vload
PCD_BSAVE            = $dd9a ; bsave
PCD_BVERIFY          = $dd9b ; bverify
PCD_POWEROFF         = $dd9c ; poweroff
PCD_RESET            = $dd9d ; reset
PCD_REBOOT           = $dd9e ; reboot
PCD_MOUSE            = $dd9f ; mouse
PCD_MB               = $dda0 ; mb
PCD_MX               = $dda1 ; mx
PCD_MY               = $dda2 ; my
PCD_MWHEEL           = $dda3 ; mwheel
PCD_RPTDOLLAR        = $dda4 ; rpt$
PCD_MOVSPR           = $dda5 ; movspr
PCD_SPRITE           = $dda6 ; sprite
PCD_SPRMEM           = $dda7 ; sprmem
PCD_ST               = $dda8 ; st
PCD_STOP             = $dda9 ; stop
PCD_SYS              = $ddaa ; sys
PCD_TDATA            = $ddab ; tdata
PCD_TATTR            = $ddac ; tattr
PCD_TILE             = $ddad ; tile
PCD_TICMD_WRITE      = $ddae ; ti.write
PCD_TIDOLLARCMD_WRITE = $ddaf ; ti$.write
PCD_WAIT             = $ddb0 ; wait
PCD_I2CPOKE          = $ddb1 ; i2cpoke
PCD_I2CPEEK          = $ddb2 ; i2cpeek
PCD_BANK             = $ddb3 ; bank
PCD_SLEEP            = $ddb4 ; sleep
PCD_FMINIT           = $ddb5 ; fminit
PCD_FMNOTE           = $ddb6 ; fmnote
PCD_FMDRUM           = $ddb7 ; fmdrum
PCD_FMINST           = $ddb8 ; fminst
PCD_FMVIB            = $ddb9 ; fmvib
PCD_FMFREQ           = $ddba ; fmfreq
PCD_FMVOL            = $ddbb ; fmvol
PCD_FMPAN            = $ddbc ; fmpan
PCD_FMPLAY           = $ddbd ; fmplay
PCD_FMCHORD          = $ddbe ; fmchord
PCD_FMPOKE           = $ddbf ; fmpoke
PCD_PSGINIT          = $ddc0 ; psginit
PCD_PSGNOTE          = $ddc1 ; psgnote
PCD_PSGVOL           = $ddc2 ; psgvol
PCD_PSGWAV           = $ddc3 ; psgwav
PCD_PSGFREQ          = $ddc4 ; psgfreq
PCD_PSGPAN           = $ddc5 ; psgpan
PCD_PSGPLAY          = $ddc6 ; psgplay
PCD_PSGCHORD         = $ddc7 ; psgchord
PCD_CLS              = $ddc8 ; cls
PCD_LOCATE           = $ddc9 ; locate
PCD_COLOR            = $ddca ; color
; ************************************************************************************************
; ************************************************************************************************
;
;		Name:		errdispatch.asm
;		Purpose:	Dispatch error handler
;		Created:	8th October 2023
;		Reviewed: 	No
;		Author:		Paul Robson (paul@robsons.org.uk)
;
; ************************************************************************************************
; ************************************************************************************************

		.section code

; ************************************************************************************************
;
;								Set error handler to YX
;
; ************************************************************************************************

SetErrorHandler:
		sty 	ErrorHandlerVector+1
		stx 	ErrorHandlerVector
		rts
		
; ************************************************************************************************
;
;								Go to current error handler
;
; ************************************************************************************************

CallErrorHandler:
		jmp 	(ErrorHandlerVector)

		.send code

		.section storage
ErrorHandlerVector:
		.fill 	2
		.send storage

; ************************************************************************************************
;
;									Changes and Updates
;
; ************************************************************************************************
;
;		Date			Notes
;		==== 			=====
;
; ************************************************************************************************
;
;	This file is automatically generated.
;
	.section code
ErrorV_range:
	jsr	CallErrorHandler
	.text	"OUT OF RANGE",0
ErrorV_value:
	jsr	CallErrorHandler
	.text	"BAD VALUE",0
ErrorV_syntax:
	jsr	CallErrorHandler
	.text	"SYNTAX ERROR",0
ErrorV_type:
	jsr	CallErrorHandler
	.text	"TYPE MISMATCH",0
ErrorV_unimplemented:
	jsr	CallErrorHandler
	.text	"NOT IMPLEMENTED",0
ErrorV_assert:
	jsr	CallErrorHandler
	.text	"ASSERT FAIL",0
ErrorV_line:
	jsr	CallErrorHandler
	.text	"UNKNOWN LINE NUMBER",0
ErrorV_internal:
	jsr	CallErrorHandler
	.text	"INTERNAL ERROR",0
ErrorV_divzero:
	jsr	CallErrorHandler
	.text	"DIVIDE BY ZERO",0
ErrorV_structure:
	jsr	CallErrorHandler
	.text	"BLOCK MISMATCH",0
ErrorV_stop:
	jsr	CallErrorHandler
	.text	"PROGRAM STOPPED",0
ErrorV_data:
	jsr	CallErrorHandler
	.text	"OUT OF DATA",0
ErrorV_undeclared:
	jsr	CallErrorHandler
	.text	"UNKNOWN ARRAY",0
ErrorV_redefine:
	jsr	CallErrorHandler
	.text	"ARRAY REDEFINED",0
ErrorV_index:
	jsr	CallErrorHandler
	.text	"BAD ARRAY INDEX",0
ErrorV_memory:
	jsr	CallErrorHandler
	.text	"OUT OF MEMORY",0
ErrorV_channel:
	jsr	CallErrorHandler
	.text	"INPUT/OUTPUT ERROR",0
ErrorV_break:
	jsr	CallErrorHandler
	.text	"BREAK PRESSED",0
ErrorV_toobig:
	jsr	CallErrorHandler
	.text	"PROGRAM TOO BIG",0
	.send code
; ************************************************************************************************
; ************************************************************************************************
;
;		Name:		forward.asm
;		Purpose:	Move object pointer forward
;		Created:	18th April 2023
;		Reviewed: 	No
;		Author:		Paul Robson (paul@robsons.org.uk)
;
; ************************************************************************************************
; ************************************************************************************************

		.section code

; ************************************************************************************************
;
;								Move objPtr forwards. CS if end
;
; ************************************************************************************************

MoveObjectForward:

		lda 	(objPtr) 					; get next
		
		cmp 	#$FF  						
		beq 	_MOFEnd

		cmp 	#$40 						; 00-3F
		bcc 	_MOFAdvance1 				; forward 1

		ldy 	#2 							; 40-6F
		cmp 	#$70 						; forward 2
		bcc 	_MOFAdvanceY 				

		cmp 	#PCD_STARTSYSTEM 			; 70 - System tokens.
		bcc 	_MOFAdvance1 				; forward 1

		tay 								; read the size.
		lda 	MOFSizeTable-PCD_STARTSYSTEM,y
		tay
		iny 								; add 1 for the system token.
		bne 	_MOFAdvanceY 				; if 0, was $FF thus a string/data skip.

		ldy 	#1 							; get length byte
		lda 	(objPtr),y
		tay 								; into Y.

		clc
		lda 	objPtr						; add 2 to the object pointer
		adc 	#2
		sta 	objPtr
		bcc 	_MOFNoCarry1
		inc 	objPtr+1
_MOFNoCarry1:		
		bra 	_MOFAdvanceY

_MOFAdvance1:
		ldy 	#1
_MOFAdvanceY:				
		tya 								; add Y to objPtr
		clc
		adc 	objPtr
		sta 	objPtr
		bcc 	_MOFNoCarry2
		inc 	objPtr+1
_MOFNoCarry2:		
		clc 								; not completed.
		rts
		;
		;		At the end so advance past $FF end marker and return CS.
		;
_MOFEnd:
		inc 	objPtr
		bne 	_MOFENoCarry
		inc 	objPtr+1
_MOFENoCarry:
		sec
		rts		

		.send code

; ************************************************************************************************
;
;									Changes and Updates
;
; ************************************************************************************************
;
;		Date			Notes
;		==== 			=====
;
; ************************************************************************************************
;
;	This file is automatically generated
;
.section code
MOFSizeTable:
	.byte	1         	; $dd .shift
	.byte	1         	; $de .byte
	.byte	2         	; $df .word
	.byte	6         	; $e0 .float
	.byte	255       	; $e1 .string
	.byte	255       	; $e2 .data
	.byte	2         	; $e3 .goto
	.byte	2         	; $e4 .gosub
	.byte	2         	; $e5 .goto.z
	.byte	2         	; $e6 .goto.nz
	.byte	2         	; $e7 .varspace
	.byte	2         	; $e8 .restore
	.byte	2         	; $e9 .fngosub
	.byte	0         	; $ea .deferror
	.byte	2         	; $eb .exitdo
	.byte	2         	; $ec .casenext
	.byte	2         	; $ed .caseend
	.byte	2         	; $ee .ifnext
	.byte	2         	; $ef .ifelse
	.byte	1         	; $f0 .unwind
	.byte	0         	; $f1 .fnsave
	.byte	0         	; $f2 .fnrestore
	.byte	0         	; $f3 .fnpush
	.byte	0         	; $f4 .fnpop
	.byte	3         	; $f5 .bgosub
.send code
