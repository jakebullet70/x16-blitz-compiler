...
#
#		The sound handlers run from bank 1 (HANDLER_BANK) and leave through .exitbank, except the four
#		that take a string, which stay in low RAM with X16_Audio_Parameters8_String
#		(docs/blitz/HANDLER-BANK.PLAN.md section 1). Each handler opens its own section, which keeps the
#		markers in order and the opcode numbers unchanged.
#
...
for s in [x.strip() for x in src if x.strip() != ""]:
	...
	apicall = m.group(2)
	setup = m.group(3)
	section = "code" if setup == "A$" else "banked"
	#
	params = "" if setup == "-" else "#,#"
	if setup == "A$":
		params = "#,@"											# the audio ROM reads the text from bank 0
