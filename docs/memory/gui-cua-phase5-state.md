...
Committed already: `2d18df6`, the six CUA modules into root `GPC-BASIC/` behind six
`*.PLAIN.INC.BL` front doors (THEME, MENUVERT, MENUBAR, LINEINPUT, GUI, GUI2). The user
approved one `LIB.GUIPLAIN.INC.BL`; six files is a reported deviation, not a mistake.
...
merge, not a copy), STRCASE.INC.BL (work copy wants a `LIB.UTILBANK.INC.BL` root lacks),
