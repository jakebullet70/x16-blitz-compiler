...
description: "For an A/B comparison against an old commit, take source/application/GPC.BIN, never drive/GPC.BIN -- the drive copy can be committed stale and will invent regressions that are not there."
metadata: 
  node_type: memory
  type: feedback
...
`make libs` writes `source/application/GPC.BIN` and copies it to `drive/GPC.BIN`, but the two
are committed independently, so **the drive copy can lag by a build**. At `3335ab5` it did:
    ...
    3335ab5:drive/GPC.BIN              17,949 bytes   940 bytes stale

**Why:** on 2026-09-06 a two-pass change was A/B'd against `git show 3335ab5:drive/GPC.BIN`
...
Worth committing `drive/GPC.BIN` alongside `source/application/GPC.BIN` so a fresh clone tests
