...
**It builds in `GPC-BASIC-TOOLS-SRC/GPC.ERR/`, which is the emulator drive.** Nothing is staged into `source/drive/`.
...
those two and installs `GPC.ERR.PRG` and `GPC.ERR.OVL` into `GPC-BASIC-TOOLS-SRC/GPC-HELP/`.
...
The build rule is also in the header of `GPC-BASIC-TOOLS-SRC/GPC.ERR/GPC.ERR.BASL` and in
`GPC-BASIC-TOOLS-SRC/GPC.ERR/readme.md`, where whoever rebuilds it will be looking.
