...
offset against, which is the honest price. `source/drive/DEFFNS.BASL` is the test: twelve formals and a
four-term last argument, called both ways, both giving 11.

Tests are `source/drive/DEFP1|DEFP1B|DEFP1C|DEFP2.BASL` and five refusals `DEFPV`..`DEFPZ`. `DEFP3` is
