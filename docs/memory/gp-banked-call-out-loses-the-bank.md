...
**The rule for a program with more than one code bank.** A `LIBBANK`-style shim is
`LABEL: BANK CODEBANK : GOSUB LABEL.BODY : RETURN`, and it deliberately does **not** restore the
caller's bank -- `LIBBANK.INC.BL` says why: a single holding variable would be overwritten by a
