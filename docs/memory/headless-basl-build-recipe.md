...
regenerated by `make libs`; take both it and `GPC.BIN` from `source/drive/`, not `source/application/`,
which holds only `GPC.BIN`.

**A shared RUN needs the runtime file too, and there are two.** `GPC.RT.<ver>.BIN` is the core
and `GPB.RT.<ver>.BIN` is the core plus the GP handlers; a program with GP code loads the second.
Copy both from `source/drive/`. A missing one prints `?RT` right after `LOADING FROM`, then `READY.`,
and the program prints nothing. Cost one cycle on 2026-09-14: a `GP.BANKED` test with only
`GPC.RT` copied.
...
**`cp GPC-BASIC/*.INC.BL source/drive/` CAN OVERWRITE THE KEYWORD FILE WITH A STALE ONE.**
...
silently downgrades `source/drive/`, and a keyword added since then STOPS BEING A KEYWORD: BASLOAD
