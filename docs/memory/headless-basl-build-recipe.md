...
regenerated by `make libs`; take both it and `GPC.BIN` from `drive/`, not `source/application/`,
...
Copy both from `drive/`. A missing one prints `?RT` right after `LOADING FROM`, then `READY.`,
...
**`cp GPC-BASIC/*.INC.BL drive/` CAN OVERWRITE THE KEYWORD FILE WITH A STALE ONE.**
...
silently downgrades `drive/`, and a keyword added since then STOPS BEING A KEYWORD: BASLOAD
