...
description: "MENUHELP works; the editor's broken dropdown was a caller that set three fields of the interface and hand-rolled the rest - and MDPROBE is the way to prove which side is wrong"
metadata: 
  node_type: memory
  type: feedback
...
it.** `MENUHELP.INC.BL` was suspected for most of a session over a dropdown that drew the
wrong height with rows missing. It was never the library.

`GPC-BASIC/MENUDEMO.EXP.BL` is MENUHELP's documented example. Copied into a work dir with
`GPB/THEME/MENUHELP.INC.BL`, with `MENUHELP.RUN` swapped for `MENUHELP.DRAW` and a VPEEK
...
`ED.DRAW.DROPDOWN` set `MENUHELP.X`, `.Y` and `.WIDTH`, then hand-rolled a `FOR` loop
poking `.DRAWROW`/`.DRAWATTR` into `MENUHELP.ROW` — the module's *lowest* entry point —
leaving `.COUNT`, `.ATTR` and `.HIATTR` at zero. `MENUHELP.ROW` reads `MENUHELP.ATTR`
...
documented inputs and calling `MENUHELP.DRAW` was **5 bytes smaller** (`OK CODE 9650` ->
`9645`).
