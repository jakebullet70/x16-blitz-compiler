...
until the workspace grows over it. `source/drive/BANKP.BASL` reports codePtr and the bank register from a
...
library entry point (`BANK n : GOSUB body : RETURN`, ~12 bytes each). `source/drive/BANKR.BASL` hangs and
`BANKS.BASL` -- the same program with the bank put back -- does not.

Tests: `source/drive/BANK*.BASL`, thirteen of them, each marked program run against an otherwise
