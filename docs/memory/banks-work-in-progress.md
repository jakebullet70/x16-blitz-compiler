...
page, so a RUN after END keeps the copy. Step 25 built the tree: `GPC.BIN` 31,892 B, build 123 image and runtime files in `source/drive/`.
...
`genx16.py`, the three generated files, `00runtime.asm`, `source/drive/testend.asm`), handler step 15 (`errors/errorhandler.asm`), handler step 17 (`common.inc`, `rtbuild.txt`,
