...
12. `source/unit-tests/gpctest.py`:
    ...
    Done 2026-09-14. The old references went to `source/scratch/gpctest/ref-old/`, and `gpctest.py ref`
...
13. GPBMODS, built in place with `modsbuild.py`: eight overlays `.004` to `.011`, with sizes as
    ...
    `source/drive/`, which is what `modsbuild.py` does. It tokenised to 73,586 bytes and compiled to
    ...
    05:13 build are still in `source/drive/`.

### Phase E: names and docs

14. `.gitignore:260` `source/drive/*.B[0-9][0-9]` becomes `source/drive/*.[0-9][0-9][0-9]`. The "nn reaches 63" comments in
    `source/gpc/modsbuild.py`, `xbasebuild.py` and `samplesbuild.py`. `release.sh:119`.

    Done 2026-09-14. `.gitignore` ignores `source/drive/*.[0-9][0-9][0-9]` and its comment names the
    ...
    `source/drive/`. The 74 stale `.Bnn` files in `source/drive/` now show as untracked.
...
16. Samples:
    - The GPBMODS ABOUT text: grep `OVERLAYS 8` and `.B07` in `GPC-BASIC-TOOLS-SRC/GPB-MODS-TESTING/GPBMODS.BASL`.
    - The `XBASE.B04` checks in `xbase-demo.bat` and the comment in `gpbmods-demo.bat`.
    - The tables in `GPC-BASIC-TOOLS-SRC/GPB-MODS-TESTING/readme.md`, plus `source/drive/readme.md`,
      `source/gpc/BUILD-HANDOFF.md` and `docs/blitz/EMBEDDED-VS-SHARED.md`.
    - The staged copies in `source/drive/` stay as they are.
    ...
    `GPC-BASIC-TOOLS-SRC/GPB-MODS-TESTING/readme.md` names the files `GPBMODS.004` to `.011` and the breakdown
    rows `.004` to `.011`. `source/drive/readme.md` lists `*.nnn`. `BUILD-HANDOFF.md` gives `.002` to
    ...
    `source/drive/GPBMODS.MAP` and `GPBMODS.SRC.SYM`, not a rename. `GUI-CUA-PLAN.md`,
    `GUI-PAGE-PLAN.md` and `GPC-BASIC-TOOLS-SRC/XBASE/` still say `.Bnn`; they are history or XBASE, and the
