...
- Sizing: the modules in `GPC-BASIC-TOOLS-SRC/GPB-MODS-TESTING` hold about 1,000 `GOSUB`, 300 `GOTO`, 930 `IF`,
...
1. **Reference set.** `chk.py` and `sweep.py` from the two-pass work are not in the repo. Write a
   script that compiles a fixed list of `source/drive/` programs and diffs each object against a stored
   copy.
...
One `source/drive/` program a case, or one program with a section a case.
