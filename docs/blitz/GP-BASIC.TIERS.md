...
**`#DEFINE` value substitution is confirmed** (`source/drive/MSEDIT/BASLOAD.MD:313-331`): *"defines a 16 bit
...
keywords. It is **staged flat into `source/drive/`** to be built, because `source/drive/` is the emulator's drive and
...
(`source/drive/MSEDIT/BASLOAD.MD`). They are allocated **downward from `$CE7F`** and never renumbered —
...
`source/drive/` is the emulator's drive and `GPC-BASIC/` holds the masters, so a build stages the files
...
- copy it and every module it includes into `source/drive/`
- `python source/gpc/build_basl.py XXX.EXP.BL XXX.PRG`, then compile the PRG with `GPC.BIN`

`source/drive/*.INC.BL` and `source/drive/*.EXP.BL` are gitignored precisely because they are staging copies;
