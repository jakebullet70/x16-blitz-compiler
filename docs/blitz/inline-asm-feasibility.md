...
> **SUPERSEDED, and it was right.** Option B was built and shipped as **`GP.ASM` / `GP.ENDASM`** —
> see §3.9 of `GPC-BASIC/GP-BASIC.md`. It went further than this note proposed: the assembler runs
> **on the X16 at compile time** rather than host-side, so the "no assembler on the X16" hard part
> was solved rather than worked around, and the body rides in `REM` statements, so no new tokeniser
> syntax was needed either. The variable ABI is `{VAR}`, resolved through BASLOAD's own `#SYMFILE`.
...
> been confirmed on the real thing: `GPC-BASIC-TOOLS-SRC/edit/` now renders a text row in **18.8**
...
> reaches the same speed and, unlike a new command, costs nothing in programs that never use it.
