...
- **Bank 1 use.** A program that writes bank 1 at run time breaks every later call to a moved handler.
  ...
  - `GMX.S.FILE` in GPBMODS selects bank 1 only to check that STASHFILE leaves it selected.
    `source/drive/RGL.BASL` and `source/drive/RGN.BASL` select it and write nothing.
  - BNK64 in `source/unit-tests/banktest3.py` compiled regions in banks 1 to 64 and expected
    ...
    which uses banks 2, 100, 254 and 255. Both sources are in the untracked `source/scratch/banktest3/`.
...
10. `BANKMGR.INIT` marks bank 1 taken in `GPC-BASIC-TOOLS-SRC/GPB-MODS-TESTING/GPC-BASIC/BANKMGR.INC.BL`. The root
    ...
    which selects the bank and writes nothing, `source/drive/RGL.BASL` and `RGN.BASL`, XBASE's two
    `BANKMGR` copies, and the staged `source/drive/BANKMGR.INC.BL`. Nothing was built.
...
14. **Entries.** `BankEnter` and `BankEnterShift` in low RAM, as in §3. They jump through step 13's
    ...
    - The runtime `build` link's object overwrote the last banked byte, an `rts` at `$3DFE`.
      `source/drive/testend.asm` put `ObjectCodePreHeader` 2 B below the page after the code, and the
...
22. The embedded bank code is a second image file, `GP1.IMG.nnn.BIN`. `genrtimage.py` names and
    ...
    `GP1.RT.nnn.BIN`, `.gitignore` ignores the `source/drive/` copy, and `dcref.py` and `banktest3.py`
...
25. `make libs`, then `make -C source/runtime gpc-rt`, in the background.
    Done 2026-09-15: both exit 0 with no errors. `source/drive/GPC.BIN` is 31,892 B,
    ...
    9,861 B) and `GP1.RT.123.BIN` ($A000, 2,432 B). The build 122 files are still in `source/drive/`.
...
28. The manual's §7 limits, the memory notes on the shared p-code cap, the runtime limits and the core
    page cushion (52 B), the `.122.` file names (`source/drive/readme.md`, `help-demo.bat`, `fntest.py` and
    ...
    `source/drive/readme.md` lists the build 123 files. `help-demo.bat`, `fntest.py` and
    `BUILD-HANDOFF.md` say `nnn`, because `GPC-BASIC-TOOLS-SRC/GPC-HELP` still holds build 122's files.
