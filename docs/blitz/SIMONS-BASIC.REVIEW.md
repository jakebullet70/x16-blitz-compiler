...
and a masked hardware `VSCROLL`. `GPC-BASIC-TOOLS-SRC/GPC-HELP` uses the first, `GPC-BASIC-TOOLS-SRC/edit` the third.
...
codes from a caller table, and `GPC-BASIC-TOOLS-SRC/edit` builds a re-ordered font.
...
| `MOD(A,B)` | `MOD(A,B)` | Already present. `x16_unary.def` has `MOD (#,#) T N`, the token table carries `$CEDE`, and `GPC-BASIC-TOOLS-SRC/edit` calls it. |
