...
`source/drive/GPC.ERR.BASL` from a two-prompt text helper into a GUI tool that answers with a file name,
a source line and the text of the offending statement.

The name does not change. `release.sh` stages `source/drive/C.GPC.ERR.PRG` as `GPC.ERR.PRG` and that
stays true.
...
|---|---|---|
| `M.<source>` | `GPC.BASL`, on the machine | `source/drive/M.GPC.ERR` |
| `M.<source>.SRC.PRG` | the same, when the source name carries the suffix | `GPC-BASIC-TOOLS-SRC/GPC-HELP/M.GPB.HELP.SRC.PRG` |
| `<name>.MAP` | `source/gpc/samplesbuild.py`, headless | `GPC-BASIC-TOOLS-SRC/GPC-HELP/GPB.HELP.MAP` |
...
`FILEPICK` matches suffixes. `source/drive/GPC.BASL` builds the name and was the one place that changed.
...
`$0801`, so it wraps on a source over 63 KB. 1,304 of the 4,791 links in `source/drive/GPBMODS.SRC.PRG`
...
bytes, against 24,847. `source/drive/GPC.ERR.PRG`, the tokenised source, is 62,494 bytes, against
51,155.
...
travel with the `.PRG`. `source/drive/GPC.ERR.PRG`, the tokenised source, is 60,710 bytes. Phase 3 cost
...
`GPC-BASIC-TOOLS-SRC/GPC-HELP` is the fixture. It has all three files from the build of 2026-09-24, and a known
answer.
...
The harness is `source/drive/GPCERRT.BASL`. It holds GPC.ERR's own resolver routines copied unchanged
with a different caller, compiles SHARED and runs headless. It is a throwaway in `source/drive/` and is
...
`source/drive/SRCTEST.BASL` prints `ERR.SRC.WORDS$` for every window it shows. The names it printed were
...
`source/drive/ERRSELF.BASL`, a generated fixed-answer variant of the real program, drove the whole chain
...
1. Map naming is `<name>.MAP`. `source/drive/GPC.BASL` builds it from the source name, cutting `.PRG` and
