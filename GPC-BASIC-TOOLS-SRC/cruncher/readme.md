...
That is the whole job. Measured on `GPC-BASIC-TOOLS-SRC/edit/EDITOR.BASL`:
...
To crunch the editor instead, point the drive at `GPC-BASIC-TOOLS-SRC\edit` and drop `CRUNCH.PRG` and
`CRUNCH.BIN` in beside `EDITOR.BASL`.

## Rebuilding

BASLOAD resolves `#INCLUDE` off the drive and `build_basl.py` uses `source\drive\` as the emulator's
filesystem root, so both sources and the two includes stage there first. `python` and `make` are
off-PATH; see `source/local.make`.

```
copy GPC-BASIC-TOOLS-SRC\cruncher\CRUNCH.BASL             source\drive\
copy GPC-BASIC-TOOLS-SRC\cruncher\CRUNCHER.BASL           source\drive\
copy GPC-BASIC-TOOLS-SRC\cruncher\GPC-BASIC\GPB.INC.BL    source\drive\
copy GPC-BASIC-TOOLS-SRC\cruncher\GPC-BASIC\STRCASE.INC.BL source\drive\
