...
otherwise be missing from the help entirely**.
...
it that the manual does not document — not just the ones in `BANNER_ONLY`. That is how a working
...
It builds in this folder, with nothing staged into `source\drive\`. The source includes its modules as
...
Measured on the bench in `source/drive/SCRLTST.BASL`, 50 slides each:
...
| PET upper/lower (3) | yes | yes | a byte is not its own tile index, so every BASLOAD literal comes out case-swapped unless the font is re-indexed first — and the re-order buries the frame glyphs. This is what `GPC-BASIC-TOOLS-SRC/edit` does |
...
while library work is in flight.

`COMBO` and `CHECK` are not called by the viewer. `GUI-DIALOGS.INC.BL` needs them: the form engine
has combo and check controls.
...
A length that disagrees does not stop the viewer.
