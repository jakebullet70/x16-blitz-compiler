...
|---|---|---|
| 4 | `THEME`, `MENUVERT`, `MENUBAR`, `LINEINPUT`, `GUI`, `GUI2` -- everything that draws | `BANKMGR.CLAIM` at startup, as `SHIM.GUIBANK` |
...
**The library modules come from `GPC-BASIC-TOOLS-SRC/GPB-MODS-TESTING/GPC-BASIC/`, not from the root library.**
That is where the banked copies live: their entry points are renamed to `.BODY` and `SHIM.GUIBANK.INC.BL`
...
copy BANKMGR.INC.BL APPSYS.INC.BL KB.INC.BL
     STRINGS.INC.BL STASH.INC.BL SHIM.GUIBANK.INC.BL
...
`GUI2.INC.BL` is in it because `SHIM.GUIBANK.INC.BL` shims `GUI.LISTBOX` and BASLOAD resolves every label
