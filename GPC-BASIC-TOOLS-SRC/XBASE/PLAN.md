...
disk, one record is visible at a time, and the whole database is never loaded. `GPC-BASIC-TOOLS-SRC/XBASE/` holds
...
```
bank 4   SHIM.GUIBANK    GUI library region        existing
...
| `DBBANK.INC.BL` | low | one shim per public entry, SHIM.GUIBANK pattern |
