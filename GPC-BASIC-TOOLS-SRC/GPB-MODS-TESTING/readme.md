...
The drive is `source/drive/`, not this folder: the object is compiled shared and loads the resident
...
copy GPC-BASIC\GPB.INC.BL                        source\drive\
...
P-code bytes, differenced out of `source/drive/GPBMODS.MAP` against `source/drive/GPBMODS.SRC.SYM`. The
...
`GPC-BASIC-TOOLS-SRC/GPC-HELP` and `GPC-BASIC-TOOLS-SRC/edit` build against.
