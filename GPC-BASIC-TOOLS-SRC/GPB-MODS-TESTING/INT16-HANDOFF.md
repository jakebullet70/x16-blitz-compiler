...
`GPC-BASIC-TOOLS-SRC/GPB-MODS-TESTING/GPC-BASIC/` first, then carried to root `GPC-BASIC/`, XBASE and GPB.HELP.
...
2. **XBASE.**
   - Settle SHIM.UTILBANK, then copy the converted shared modules into `GPC-BASIC-TOOLS-SRC/XBASE/GPC-BASIC/`.
...
- GPC-BASIC-TOOLS-SRC/edit (APPSYS, GUI, LINEINPUT, MENUBAR, MENUVERT, STASH, STRCASE, THEME), GPC-BASIC-TOOLS-SRC/color-test
  (APPSYS, THEME) and GPC-BASIC-TOOLS-SRC/cruncher (KB, STRCASE, STRINGS) include their own older copies.
...
- **Names and file list** come from `source/drive/<PROGRAM>.SRC.SYM`, which strips suffixes. The suffixes
...
- **Variable space and workspace** are read from `source/drive/<PROGRAM>.PRG`: the bootstrap's two page
