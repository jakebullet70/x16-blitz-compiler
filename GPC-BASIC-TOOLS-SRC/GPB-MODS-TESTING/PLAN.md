...
`GPC-BASIC-TOOLS-SRC/GPB-MODS-TESTING/GPC-BASIC/` is the working copy. Root `GPC-BASIC/` is the release copy.
Work happens here; a module goes to the root when it passes its panel.

Drift is already real and is the reason to state the direction. `GPC-BASIC-TOOLS-SRC/edit/GPC-BASIC/` holds
...
- The root copy is what `GPC-BASIC-TOOLS-SRC/GPC-HELP` and `GPC-BASIC-TOOLS-SRC/edit` build against. Copy to the root and
...
```
GPC-BASIC-TOOLS-SRC/GPB-MODS-TESTING/
    PLAN.md          this file
...
file name. It cost the first build of this project. `GPC-BASIC-TOOLS-SRC/color-test/COLORTST.BASL` has the same
...
The drive is `source/drive/`, as `color-demo.bat` does, because the object is compiled shared and
`GPC.RT.<ver>.BIN` lives there.

```
copy GPC-BASIC-TOOLS-SRC/GPB-MODS-TESTING/GPBMODS.BASL       -> source/drive/
copy GPC-BASIC-TOOLS-SRC/GPB-MODS-TESTING/GPC-BASIC/*.INC.BL -> source/drive/
...
| `GP.BOX`, `GP.FILL`, `GP.PRINTAT`, `GP.CHAR` | runtime handlers, resident in low RAM |
...
Eleven tests in `source/drive/BANK*.BASL`, each marked program run against an identical unmarked control:
...
Thirteen tests, `source/drive/BANK*.BASL`. `GPC.BIN` 17,269 -> 17,694.
...
Nineteen test sources in `source/drive/BANK*.BASL`. Thirteen are the pass/fail suite; the rest are the
