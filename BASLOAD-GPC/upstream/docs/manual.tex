...
\section{Introduction}
    ...
    \begin{itemize}
        ...
        \item Line numbers are not used in the source code; named labels are 
              declared as targets for GOTOs and GOSUBs.
    ...
    based version of BASLOAD is deprecated.
...
\section{Hello World Example}
    ...
    If the result is ?SYNTAX ERROR, it means that BASLOAD is not
    present in your ROM.
...
\section{BASIC Syntax}
    ...
    \subsection{General}
        ...
        \begin{itemize}
            ...
            \item Labels are declared as targets for GOTOs, GOSUBs and
        ...
        When translating code written for the built-in BASIC, note that BASLOAD distinguishes between 
        variable names based on the first 64 characters. A program written for the built-in BASIC can refer 
        to the same variable using names of different lengths. For instance TI\$ and TIME\$ both return the 
        current time in the built-in BASIC, and DA\$ and DATE\$ both return the current date{\textemdash}while BASLOAD 
        will treat TIME\$ and DATE\$ as different variables than TI\$ and DA\$. When writing BASLOAD programs, 
        always use the short forms TI\$ and DA\$ directly if you want the current time or date.
    ...
    \subsection{Variables}
        ...
        Variables are automatically declared when they are first used in
        code.
...
\section{BASLOAD Options}
    ...
    #<NAME> <ARGUMENT 1> ... <ARGUMENT n>
        ...
        Arguments may be numeric values, identifier names, or strings. 
        A string may optionally be enclosed in double quotes, which makes it possible 
        for it to contain whitespace characters.

    \subsection{Option: \#\# Comment}

        This option is an alternative comment that is never included with the generated
        code.
    ...
    \subsection{Option: \#AUTONUM \textless int8 \textgreater }
        ...
        room to insert code directly into the generated code,
    ...
    \subsection{Option: \#CONTROLCODES 0 $|$ 1}
        ...
        The control codes are all typed within curly brackets. There
        is a list of the control codes in Appendix A. 
        ...
        The named control codes are only available
        within a string.

        If you want to type a left curly bracket or a 
        backslash in a string, you need to escape each of them with 
        a backslash while this option is active.

        While the option is active it is also possible
        to type in characters by their HEX code. The HEX code is
        prefixed by "\textbackslash X". For instance, "\textbackslash Xff" outputs the PI character
        in PETSCII upper case mode.
    ...
    \subsection{Option: \#DEFINE \textless identifier name\textgreater \  \textless int16\textgreater }
        ...
        used for BASIC reserved words, variables, or labels.
    ...
    \subsection{Conditional Translation of Source Code}
        ...
        such as BASIC reserved words, variables, and labels, are ignored by IFDEF.
