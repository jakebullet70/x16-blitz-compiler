...
`GP.INC.BL` defines **no variables at all**. It is 29 `#TOKEN` lines and nothing else. Everything
...
`GP.MENU` returned). They are tokens rather than variables for a hard reason: **nothing in the
...
| internal | `BMX.PTR` `BMX.HEADER$` `BMX.SCRATCH` `BMX.SCRATCH$` `BMX.PALBASE` `BMX.ADDR` `BMX.LO` `BMX.HI` `BMX.BANK` `BMX.REST` `BMX.COUNT` `BMX.CHUNK` `BMX.GOT` `BMX.STEP` `BMX.SKIP` `BMX.ROW` `BMX.ROWS` `BMX.X0` `BMX.Y0` `BMX.BAD` |
| constants | `BMXK.MACPTR` `BMXK.CHKIN` `BMXK.CLRCHN` `BMXK.VCTRL` `BMXK.VLO` `BMXK.VMID` `BMXK.VHI` `BMXK.PORTLO` `BMXK.PORTHI` `BMXK.LFN` |
...
meant to call. `BMX.STREAM.MORE`, `INPHELP.REDRAW` and `THEME.LOAD.DARK` are internal, and a
`GOSUB` to one will do something, just not something useful.

Each module also has a skip label it jumps over itself with — `THEME.SKIP`, `APPHELP.SKIP`,
`STRHELP.SKIP`, `BMX.MODULE.END`, `INPHELP.MODULE.END`. Those exist so an include can sit anywhere in
the file, the top included. **Do not branch to one.**
...
**A dotted name whose tail is a reserved word is fine.** `GP.MENU`, `THEME.CLR`, `INPHELP.LEN` and
`INPHELP.RETURN` all contain keywords and all work, because BASLOAD matches the whole identifier. An
