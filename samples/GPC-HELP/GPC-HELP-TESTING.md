...
- **BASL MODULES**
  ...
  - [DOS.INC.BL -- USE THIS INSTEAD OF FILEIO.INC, SMALLER, TRYING TO JUST DO X16 DOS](#dosincbl----use-this-instead-of-fileioinc-smaller-trying-to-just-do-x16-dos)
  - [FILEPICK.INC.BL -- a popup file picker whose list lives in a bank.](#filepickincbl----a-popup-file-picker-whose-list-lives-in-a-bank)
  - [GUI-DIALOGS.INC.BL -- every canned dialog as a one-line call.](#gui-dialogsincbl----every-canned-dialog-as-a-one-line-call)
...
## DOS.INC.BL -- USE THIS INSTEAD OF FILEIO.INC, SMALLER, TRYING TO JUST DO X16 DOS


*From the banner header of `DOS.INC.BL`.*
...
*From the banner header of `GUI-DIALOGS.INC.BL`.*
...
```
  ...
  carries into this one. Five things are kept: the bank DLGRESET was given,
  which is where the covered cells are saved, and the DLGSHADOW, DLGSHADOWCLR,
  DLGSTYLE and DLGGLYPH settings.
  ...
  DLGSTYLE style                      the frame every box after it draws:
                                      0 solid, 1 single line, 2 rounded,
                                      3 thick. DLGGLYPH overrides it
...
*See also: 4.12 GUI-DIALOGS.INC.BL -- every dialog as a verb, 4.1 THEME.INC.BL -- named colour roles, 4.6 MENU.INC.BL -- menus built a row at a time, 4.4 LINEINPUT.INC.BL -- a positioned entry field, 4.11 GUI.INC.BL -- the box that puts the screen back, and the form in , 4.17 COMBO.INC.BL -- a drop-down list that folds into one row, 4.21 CHECK.INC.BL -- a check box, [X] or [ ], STASH.INC.BL -- save a text rectangle, and put it back.*
