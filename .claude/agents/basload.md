...
Its own manual is `source/drive/MSEDIT/BASLOAD.MD`. **It is wrong in at least one place** (see digits,
...
- **They do not nest.** A guard inside a guard reports `ERROR: ENDIF WITHOUT IF IN <file>:<line>`
  and writes the 6-byte PRG. Use **flat** symbols — `GPC-BASIC-TOOLS-SRC/edit/EDITOR.BASL` uses three
...
and must write back what it found. `EDITOR.BASL` and `GPC-BASIC/` are LF, `source/drive/GPC.BASL` is
...
| | |
|---|---|
| BASLOAD's own manual (partly wrong) | `source/drive/MSEDIT/BASLOAD.MD` |
